...
class ICTChargeLoggerGUI:
    ...
    def start_logging(self,remote=False):
       # single shot mode
        ...
        if not remote:
            # 🔄 Restore chronological layout for live mode tracker
            self.ax_charge.set_title("Integrated Charge vs Time", fontsize=22, fontweight='regular', pad=20)
            self.ax_charge.set_xlabel("Time [HH:MM:SS]", fontsize=22, fontweight='regular', labelpad=16)
            self.ax_charge.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M:%S'))
            self.ax_charge.xaxis.set_major_locator(mdates.AutoDateLocator())
            self.cursor_line.set_visible(True) # Make tracking timeline line visible again
            
            # Reset local running cache memories
            self.times.clear()
            self.charges.clear()
            self.avg_charges.clear()
    ...
    def acquire_single_shot_segments(self):
        ...
        try:
            ...
            # --- Arrays to hold plotting metrics ---
            shot_offsets = []
            shot_charges = []
        
            # We loop over the dynamic loop range assigned by the user
            offsets_to_run = list(range(-1, -(arm_number+1), -1))
            
            for offset in offsets_to_run:
                ...
                calculated_charge = charge if charge is not None else 0.0
                ...
                self.single_shot_results.append({
                    ...
                    "offset_num": offset # store for tracking reference
                })
                
                # Append data for the updated plot feature
                shot_offsets.append(offset)
                shot_charges.append(calculated_charge)
                ...
                if save_dir and shot_writer:
                    ...
                    shot_writer.writerow([shot_name, f"{calculated_charge:.3e}", f"{pulse_width_ns:.2f}"])

                print(f"Segment {offset}: {calculated_charge:.3e} C")
            ...
            # =================================================================
            # 📊 NEW PLOTTING CODE FOR LEFT AXIS (Charge vs Pulse Offset Number)
            # =================================================================
            self.ax_charge.xaxis.set_major_locator(matplotlib.ticker.AutoLocator())
            self.ax_charge.xaxis.set_major_formatter(matplotlib.ticker.ScalarFormatter())
            self.ax_charge.xaxis.set_minor_locator(matplotlib.ticker.NullLocator())
            
            # 2. Update descriptive labels
            self.ax_charge.set_title("Single Shot: Charge vs Offset", fontsize=22, fontweight='regular', pad=20)
            self.ax_charge.set_xlabel("Pulse Offset Number", fontsize=22, fontweight='regular', labelpad=16)
            
            # Reset label rotation to horizontal
            for label in self.ax_charge.get_xticklabels():
                label.set_rotation(0)
                label.set_horizontalalignment('center')
            
            # 3. Process plot data
            x_plot_data = [abs(o) for o in shot_offsets] # Converts [-1, -2, -3] to [1, 2, 3]
            self.line_avg_charge.set_data(x_plot_data, shot_charges)
            
            # Clear historical continuous line data 
            self.line_charge.set_data([], []) 
            
            # Hide the timeline tracking cursor safely 
            self.cursor_line.set_visible(False)
            self.cursor_text.set_text("")
            
            # 4. HARD ENFORCE limits from 0 to maximum shot number (arm_number)
            self.ax_charge.set_xlim(0, len(x_plot_data) + 0.5)
            
            # Auto-scale Y height view to fit the dataset securely
            self.ax_charge.relim()
            self.ax_charge.autoscale_view(scalex=False, scaley=True) # Freeze X, autoscale Y
            self.canvas.draw_idle()
            # =================================================================
