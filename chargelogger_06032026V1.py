...
# ---------------- CONFIG ----------------
WAVEFORM_SAVE_LIMIT =10     # 1 for one file, 2 for two, etc.
...
def compute_auto_charge(time_s, voltage, sensitivity):
    ...
    # If negative pulse, switch to argmin logic instead
    # peak_idx = np.argmin(voltage)
    # peak_val = voltage[peak_idx]

    amplitude = peak_val   # assuming baseline already near 0
    ...
    # # pulse_width_ns = 100
    # # t_start, t_end, fwhm = find_pulse_bounds_shortpulse(voltage, time_s, pulse_width_ns, baseline=0.0)
    ...
    # -------------------------------
    # Low-amplitude pulses: 20–80% window, no correction
    # -------------------------------
    
    if amplitude < 1.0e-3:
        t_start, t_end, fwhm = find_pulse_bounds_shortpulse(voltage, time_s, baseline=0.0)
...
class ICTChargeLoggerGUI:
    ...
    def _update_step(self):
        ...
        #     # Choose expected pulse location
        #     # Option A: known location from setup
        #     expected_idx = len(v_work) // 2
        
        #     # Option B: use first shot as reference
        #     ref_shot = self.low_charge_buffer[0]
        #     expected_idx = np.argmax(ref_shot)
        
        #     aligned_shots = []
        #     peak_indices = []
        
        #     for shot in self.low_charge_buffer:
        #         peak_idx = find_peak_idx_windowed(
        #             shot,
        ...
        #         shift = expected_idx - peak_idx
        #         shot_aligned = shift_signal_zero_pad(shot, shift)
        #         aligned_shots.append(shot_aligned)
        ...
        if self.low_charge_mode:
            ...
            min_fwhm_ns = 20.0          # reject narrower than this
            max_fwhm_ns = 300.0         # optional upper limit
            max_peak_shift = 15         # max allowed shift in samples
            ...
            for shot in self.low_charge_buffer:
                # Find pulse width
                t_start, t_end, fwhm = find_pulse_bounds_fwhm(shot, self.time_axis, smooth=True)
                ...
                # Reject very narrow noise spikes
                if fwhm_ns < min_fwhm_ns:
                    rejected_count += 1
                    continue
        
                # # Optional: reject abnormally wide pulses
                # if fwhm_ns > max_fwhm_ns:
                #     rejected_count += 1
                #     continue
        
                # Find peak near expected pulse location
                peak_idx = find_peak_idx_windowed(
                    shot,
                    expected_idx=expected_idx,
                    search_half_width=20,
                ...
                # Reject if peak is too far from expected position
                if abs(peak_idx - expected_idx) > max_peak_shift:
                    rejected_count += 1
                    continue
        
                valid_shots.append((shot, peak_idx))
        
            # If no valid shots remain, skip this cycle
            if len(valid_shots) == 0:
                print("⚠️ All shots rejected in low-charge averaging")
                self.low_charge_buffer.clear()
                self._schedule_update()
                return
        
            aligned_shots = []
            for shot, peak_idx in valid_shots:
                shift = expected_idx - peak_idx
                shot_aligned = shift_signal_zero_pad(shot, shift)
                aligned_shots.append(shot_aligned)
            ...
            print(f"Low-charge averaging: used {len(valid_shots)}, rejected {rejected_count}")
