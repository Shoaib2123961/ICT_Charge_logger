...
def adc_to_voltage(adc_counts):
    ...
    return (adc_counts* ADC_FULL_SCALE/ (2**EFF_ADC_BITS))
...
class ICTChargeLoggerGUI:
    def __init__(self, master):
        ...
        # --- Attenuation Control ---
        tk.Label(ctl_frame, text="Attenuation [10–31]:", font=self.font_label, bg="#2E86C1", fg="white").pack(side=tk.LEFT, padx=(15, 0))
    ...
    def on_attenuation_change(self, event=None):
        ...
        try:
            ...
            if not (10 <= att <= 31):
                tk.messagebox.showwarning("Invalid Input", "Attenuation must be between 10 and 31.")
                return
        except ValueError:
        ...
        try:
            ...
            print(f"✅ Updated EFF_ADC_BITS = {EFF_ADC_BITS:.3f}")
        except Exception as e:
    ...
    #  un comment if want fixed squeezing time window
        ...
        avg_charge = np.mean(self.charges[-10:])
